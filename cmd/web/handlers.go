...
func (app *application) viewCompetitor(w http.ResponseWriter, r *http.Request) {
	competitorID, err := strconv.Atoi(r.PathValue("id"))
	if err != nil || competitorID < 1 {
		http.NotFound(w, r)
		return
	}

	// Use the CompetitorModel's Get() method to retrieve
	// the data for a specific record based on its ID. If none found,
	// return a 404 Not Found response.

	competitor, err := app.competitors.Get(competitorID)
	if err != nil {
	...
	// Write the competitor data as a plain-text HTTP response body.
	fmt.Fprintf(w, "%+v", competitor)
}
